...
use esp_practice_v3::{_delay_ms, send_data, read_command, collect_data, Reading, Status};
...
fn main() -> ! {
    ...
    //create scd41
    let mut scd41: Scd4x<I2c<'_, esp_hal::Blocking>, Delay> = Scd4x::new(i2c, Delay::new());

    //Create uart for comms
    ...
    loop {
        ...
        match r {
            0 => {},//No uart connection
            1 => {
                match scd41.start_periodic_measurement() {
                    Ok(_) => (),
                    Err(_) => (),
                }
                e_status = Status {//Start experiment
                    recording: true,
                    frequency: e_status.frequency,
                    start_time_ms: Instant::now().duration_since_epoch().as_millis(),
                }
            },
            2 => e_status.recording = false,//Stop experiment
            3 => send_data(&mut uart, &mut p, &readings),//Experiment underway and data is ready to be read
            4..=255 => e_status.frequency = 4/r as u16,//Changing frequency
        }
        if !e_status.recording {continue;}//Does not record data if not recording
        collect_data(&mut readings, &e_status, &mut uart, &mut s, &mut scd41);//Collects data and adds it to readings
    }
}